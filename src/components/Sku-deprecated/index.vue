  ...
  <div class="vue-sku">
    ...
    <el-card class="box-card">
      <section>
        <div v-for="(item, index) in specification" :key="index" class="spec-line">
          <div>
            ...
            <el-input size="small" style="width:200px;" v-if="cacheSpecification[index].status"
                      v-model="cacheSpecification[index].name" placeholder="请输入规格名称"
          ...
          <div class="specification-group">
            ...
            <el-input size="small" style="width:200px;" v-model="addValues[index]"
                      placeholder="多个规格属性用空格隔开"
        ...
        <div class="add-spec">
          <el-button size="small" type="primary" :disabled="specification.length >= 5" @click="addSpec">添加新规格
          </el-button>
        </div>
      </section>
    </el-card>

<!--    &lt;!&ndash; specification display &ndash;&gt;-->
<!--    <el-card class="box-card">-->
<!--      <div slot="header" class="clearfix">-->
<!--        <span>Specification Display</span>-->
<!--        <el-button style="float: right; padding: 3px 0" type="text" @click="specificationStatus = !specificationStatus">-->
<!--          {{ !specificationStatus ? 'show' : 'hide' }}-->
<!--        </el-button>-->
<!--      </div>-->
...
<!--          <th style="width: 160px;">商品编码</th>-->
          <th>{{ tdName.childProductCost }}（元）</th>
          <th>{{ tdName.childProductStock }}</th>
          <th>{{ tdName.childProductPrice }}（元）</th>
          <th>{{ tdName.childProductWeight }}（kg）</th>
<!--          <th>启用</th>-->
        </tr>
        </thead>
        <tbody v-if="specification[0] && specification[0].value.length" id="specification">
        <tr
          :key="index"
          ...
          <template v-for="(n, specIndex) in specification.length">
            <td
              ...
              :key="n"
              :rowspan="countSum(n)" style="width:100px;">
              {{getSpecAttr(specIndex, index)}}
            </td>
          </template>
<!--          <td>-->
...
<!--          </td>-->
          <td>
            <el-input
              size="small"
              type="number"
              v-model="childProductArray[index].childProductCost"
              :placeholder="tdName.childProductCost"
              :min="0"
              :max="1000000"
              class="childProductCost"
          ...
          <td>
            <el-input
              ...
              :placeholder="tdName.childProductStock"
              :min="0"
              :max="1000000"
              class="childProductStock"
              :disabled="!childProductArray[index].isUse">
            </el-input>
          </td>
          <td>
            <el-input
              size="small"
              type="number"
              v-model="childProductArray[index].childProductPrice"
              :placeholder="tdName.childProductPrice"
              :min="0"
              :max="1000000"
          ...
          <td>
            <el-input
              size="small"
              type="number"
              v-model="childProductArray[index].childProductWeight"
              :placeholder="tdName.childProductWeight"
              :min="0"
              :max="1000000"
              class="childProductWeight"
              :disabled="!childProductArray[index].isUse">
            </el-input>
          </td>
<!--          <td>-->
<!--            <el-switch v-model="childProductArray[index].isUse"-->
        ...
        <tr>
          <td colspan="8" class="wh-foot">
            ...
            <template v-if="isSetListShow">
              <el-button @click="openBatch('childProductCost')" size="mini">进货价</el-button>
              ...
              <el-button @click="openBatch('childProductWeight')" size="mini">重量</el-button>
            </template>
            <template v-else>
              <el-input size="mini" style="width:200px;" v-model.number="batchValue"
                        placeholder="输入数字"></el-input>
              <el-button type="primary" size="mini" @click="setBatch"><i class="set-btn blue el-icon-check"></i>
              </el-button>
              <el-button type="danger" size="mini" @click="cancelBatch"><i class="set-btn blue el-icon-close"></i>
              </el-button>
            </template>
          </td>
        </tr>
...
<script>
  ...
  * */
  ...
  function objEquals(object1, object2) {
    ...
    // For the first loop, we only check for typess
  ...
  export default {
      ...
      tdName: { // 各个标题名称
        ...
        default: () => {
          return {
            ...
            childProductPrice: '市场价',
            childProductCost: '税后供货价',
    ...
    methods: {
      ...
      */
      ...
      checkData() {
        let hasError = false
        const dom = document.getElementById('specification')
        const setError = (name, index, status) => {
          if (status) {
            dom.getElementsByTagName('tr')[index].getElementsByClassName(name)[0].classList.remove('el-error')
          } else {
            dom.getElementsByTagName('tr')[index].getElementsByClassName(name)[0].classList.add('el-error')
            this.$message.error(`${this.tdName[name]}设置有误，请注意修改。`)
            hasError = true
          }
        }
        this.childProductArray.forEach((value, index) => {
          if (value.childProductStock === '') {
            setError('childProductStock', index, false)
            return
          }
          if (value.childProductPrice === '') {
            setError('childProductPrice', index, false)
            return
          }
          if (value.childProductCost === '') {
            setError('childProductCost', index, false)
            return
          }
          if (value.childProductWeight === '') {
            setError('childProductWeight', index, false)
            return
          }
          // 判断是不是数字
          if (isNaN(Number(value.childProductStock))) {
            setError('childProductStock', index, false)
            return
          }
          if (isNaN(Number(value.childProductPrice))) {
            setError('childProductPrice', index, false)
            return
          }
          if (isNaN(Number(value.childProductCost))) {
            setError('childProductCost', index, false)
            return
          }
          if (isNaN(Number(value.childProductWeight))) {
            setError('childProductWeight', index, false)
            return
          }
          // 判断字符串
          const childProductPrice = String(value.childProductPrice).split('.')
          if (childProductPrice.length > 2 || (childProductPrice[1] && childProductPrice[1].length > 2)) {
            setError('childProductPrice', index, false)
            return
          }
          const childProductCost = String(value.childProductCost).split('.')
          if (childProductCost.length > 2 || (childProductCost[1] && childProductCost[1].length > 2)) {
            setError('childProductCost', index, false)
            return
          }
          const childProductWeight = String(value.childProductWeight).split('.')
          if (childProductWeight.length > 2 || (childProductWeight[1] && childProductWeight[1].length > 2)) {
            setError('childProductWeight', index, false)
            return
          }
          // 库存不能有小数
          const childProductStock = String(value.childProductStock).split('.')
          if (childProductStock.length > 1) {
            setError('childProductStock', index, false)
            return
          }
          // 判断数值大小
          if (Number(value.childProductStock) < 0 || Number(value.childProductStock) >= this.number.stock) {
            setError('childProductStock', index, false)
            return
          }
          if (Number(value.childProductPrice) < 0 || Number(value.childProductPrice) >= this.number.market) {
            setError('childProductPrice', index, false)
            return
          }
          if (Number(value.childProductCost) < 0 || Number(value.childProductCost) >= this.number.cost) {
            setError('childProductCost', index, false)
            return
          }
          if (Number(value.childProductWeight) < 0 || Number(value.childProductWeight) >= this.number.weight) {
            setError('childProductWeight', index, false)
            return
          }
          setError('childProductStock', index, true)
          setError('childProductPrice', index, true)
          setError('childProductCost', index, true)
          setError('childProductWeight', index, true)
        })
        if (hasError) {
          window.scrollTo(0, document.getElementsByClassName('el-error')[0].getBoundingClientRect().top)
        }
        if (this.min > this.childProductArray.length) {
          this.$message.error(`商品规格数不能少于${this.min}条。`)
          return true
        }
        return hasError
      },
      // 生成 cacheSpecification 缓存参数
      generateCacheSpecification(specsGroupDtos) {
        const cacheSpecification = [];
        (specsGroupDtos || []).forEach(item => {
          cacheSpecification.push({
            name: item.name,
            status: false
          })
        })
      ...
      getData() {
        return {
          specsGroupDtos: this.specification,
          specsValueDtos: this.childProductArray
        }
      },
      setData(specsGroupDtos, specsValueDtos) {
        this.generateCacheSpecification(specsGroupDtos)
        this.specification = specsGroupDtos
        this.childProductArray = specsValueDtos
      }
    }
  }
</script>
