<template>
  <div class="album-wrapper">
    <template v-if="['video', 'image', 'audio'].includes(fileType)">
      <vue-draggable element="div" v-model="filesList" class="album-list">
        <div class="album-item" v-for="(item, index) in filesList" :key="index">
          <div class="item-content">
            ...
            <audio class="item-thumbnail" v-if="fileType === 'audio'" :src="item.fileUrl"></audio>
...
<!--          <div class="item-name" v-if="limit !== 1">{{ index === 0 ? '封面图' : ''}}</div>-->
    ...
    <template v-if="fileType === 'audio' && isPreview">
      <video-viewer
        :initial_index="currentIndex"
        v-show="showViewer"
        :on-close="handleClosePreview"
...
<script>
  ...
  import { deleteFileRequest, uploadFileToOSS, getFileUrl } from '@/api/uploadFile'
  ...
  export default {
    ...
    methods: {
      ...
      uploadRequest(data) {
        ...
        let fileType = data.file.type.split('/')[1]
        let fileName = data.file.name.split('.')[0]
        ...
        uploadFileToOSS(formData).then(res => {
          if (res.code === 200) {
            if (res.result.needWait) { // 大文件
              this.timer = setInterval(() => {
                getFileUrl(res.result.asynKeyStr).then(res => {
                  if (!res.result.needWait) {
                    if (this.fileType === 'image') {
                      ...
                      img.onload = function() {
                        if (_this.filesList.length >= _this.limit) {
                          _this.$message.error(`最多可上传${_this.limit}个文件`)
                        } else {
                      ...
                      if (this.filesList.length >= this.limit) {
      ...
      //       fileName: `${getTimestamp()}_${randomInteger(0, 1000)}_${nameArray[0]}`, // nameArray[0]
