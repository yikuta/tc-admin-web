      ...
      <el-card class="search-content">
        ...
        <el-table :data="tableData"
          ...
          <template v-if="tableAttrs.columns && tableAttrs.columns.length > 0">
            ...
            <el-table-column v-for="(column, columnIndex) in tableAttrs.columns.filter(
  ...
  methods: {
    ...
    async getList() {
      ...
      try {
        ...
        const {
          result: { totalCount, itemVOs }
        } = await this.apiFn(params)
        let res = itemVOs || []
        if (this.filterOut) {
