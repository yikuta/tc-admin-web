...
const searchTool = (form) => {
  ...
  /*
    ...
    Object.keys(formObj).map(key => {
     ...
     console.log(key, formObj[key], (!formObj[key].value && formObj[key].value !== false), (formObj[key].value && !formObj[key].value.length) && formObj[key].type !== 'boolean' && formObj[key].type !== 'Integer')
     if ((!formObj[key].value && formObj[key].value !== false) || (!formObj[key].value.length) && formObj[key].type !== 'boolean' && formObj[key].type !== 'Integer') return delete formObj[key]
   }) */
  /* let formObj = Object.assign({}, form)
  Object.keys(formObj).map(key => {
    if ((!formObj[key].value && formObj[key].value !== false && formObj[key].value !== 0) || (!formObj[key].value.length) && formObj[key].type !== 'boolean' && formObj[key].type !== 'Integer') return delete formObj[key]
  })*/
  const formArray = []
