  ...
  <div class="app-container">
    <el-card class="search-content">
      ...
      <div class="search-btn">
        <el-button type="primary" size="mini" @click="hanldeBatch" v-np="'SEC_KILL_LAUNCH_BATCH_AUDIT'">批量处理</el-button>
    ...
    <el-table :data="tableData" size="mini" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="channelType.describe" label="投放业务"></el-table-column>
      <el-table-column prop="day" label="秒杀日期"></el-table-column>
      <el-table-column prop="channelCommodityNum" label="秒杀商品数"></el-table-column>
      <el-table-column prop="totalAmount" label="优惠金额"></el-table-column>
      <el-table-column label="查看详情">
        <template slot-scope="scope">
          <el-button type="text" @click="gotoDetail(scope.row)" v-np="'SEC_KILL_LAUNCH_PRODUCT_BATCH_VIEW'">点击查看</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="auditTime" label="发起时间">
        <template slot-scope="scope">
          {{scope.row.auditTime | format('yyyy-MM-dd HH:mm:ss')}}
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作">
        <template v-slot:default="{ row }">
          <el-link v-show="row.auditStatus.value == 1" :disabled="disabledBtn" type="primary" @click="pass(row)" v-np="'SEC_KILL_LAUNCH_BATCH_PASS'">通过</el-link>
          <el-link v-show="row.auditStatus.value == 1" :disabled="disabledBtn" type="primary" @click="reject(row)" v-np="'SEC_KILL_LAUNCH_BATCH_REJECT'">驳回</el-link>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  import { reqUserList } from '@/api/coupon'
  import { reqQueryAuditList, reqBatchAudit } from '@/api/seckill'
  export default {
    name: 'man-made',
    components: {
      Pagination
    ...
    methods: {
      ...
      gotoDetail(item) {
        ...
        let query = {
          ...
          seckillLaunceCode: item.sourceCode
        }
