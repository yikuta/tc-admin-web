<template>
  <div class="app-container">
    ...
    <div class="container">
      <p>活动名称：{{$route.query.groupBuyActivityName}}</p>
      <p>活动ID：{{$route.query.groupBuyActivityCode}}</p>
      <p>拼团商品：{{$route.query.channelCommodityName}}</p>
      <p>拼团价：{{$route.query.groupBuyPrice}}</p>
      <p>操作日志</p>
...
export default {
  ...
  methods: {
    ...
    handleDataList() {
      reqPromotionLogList({
        identifier: 'group_buy_activity_' + this.$route.query.groupBuyActivityCode,
    ...
    handleLink(version) {
      const { groupBuyActivityCode } = this.$route.query
      let routeUrl = this.$router.resolve({
        path: '/active/shopping/group_booking_form_show',
        query: {
          type: 'show',
          version: version,
          groupBuyActivityCode
        }
      })
