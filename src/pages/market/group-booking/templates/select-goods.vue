        ...
        <el-tab-pane label="可选商品" name="canAdd">
          <el-card class="search-content">
            <el-form ref="searchForm" :model="searchForm" label-width="100px" size="mini">
              <el-form-item label="所属业务" prop="businessNo.value">
                <el-select class="input-width" placeholder="请选择所属业务" v-model="searchForm.businessNo.value" clearable @change="businessNoChange">
                  <el-option label="快递到家" :value="1"></el-option>
                  <el-option label="爆品预售" :value="2"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="基础分类" prop="category.value" v-show="searchForm.businessNo.value == 2">
                <el-cascader
                size="mini"
                v-model="searchForm.category.value"
                :options="pre_sale" :props="optionProps"></el-cascader>
              </el-form-item>
              <el-form-item label="基础分类" prop="category.value" v-show="searchForm.businessNo.value == 1">
                <el-cascader
                size="mini"
                v-model="searchForm.category.value"
                :options="logistics" :props="optionProps"></el-cascader>
              </el-form-item>
          ...
          <el-table :data="canAddGoodsData" ref="multipleTable" style="width: 100%" :cell-style="{ textAlign:'center' }" :header-cell-style="{ textAlign:'center' }" size="mini">
            ...
            <el-table-column label="是否地推" prop="channelCommoditySub.offLine">
              <template slot-scope="scope">
                <span>{{scope.row.channelCommoditySub.offLine ? '是' : '否'}}</span>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    search() {
      // 查询
      if (this.searchForm.businessNo.value === '') {
        this.$message({
          message: '请先选择业务类型',
          type: 'error'
        })
    ...
    businessNoChange() {
      ...
      // this.$emit('businessNo', val)
    },
    getTemplateRow(index, row) {
      this.templateSelection = row
      this.$emit('on-select-goods', this.templateSelection)
    },
