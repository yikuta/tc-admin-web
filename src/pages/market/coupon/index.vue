    ...
    <el-table :data="tableData" size="mini">
      ...
      <el-table-column prop="store" label="适用门店">
        <template slot-scope="scope">
          ...
          <el-button v-else type="text" @click="showItemSelected('depot', scope.row)" v-np="'COUPON_APPLICABLE_SHOP_FIND'">点击查看</el-button>
        </template>
      </el-table-column>
      <el-table-column prop="goods" label="适用商品">
        <template slot-scope="scope">
          ...
          <el-button v-else type="text" @click="showItemSelected('goods', scope.row)" v-np="'COUPON_APPLICABLE_PRODUCT_FIND'">点击查看</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="totalAmount" label="优惠券总额">
        <template slot-scope="scope">
      ...
      <!-- <el-table-column label="使用时间" width="200px">
        <template slot-scope="scope">
          <div v-if="scope.row.availableTimeSetting.availableTimeType === 'FIXED_TIME_PERIOD'">
            <div>{{scope.row.availableTimeSetting.fixedTimeList[0]}}</div>
            <div>至</div>
            <div>{{scope.row.availableTimeSetting.fixedTimeList[0]}}</div>
          </div>
          <div v-else-if="scope.row.availableTimeSetting.availableTimeType === 'FIXED_DURATION'">
            <div>自领取时起{{scope.row.availableTimeSetting.fixedDurationNum}}{{scope.row.availableTimeSetting.timeUnit === 'HOUR' ? '小时' : '天'}}</div>
          </div>
          <div v-else>
            <div v-if="scope.row.availableTimeSetting.fixedPeriodType === 'VALID_FOR_THE_DAY'">领取当日有效</div>
            <div v-else-if="scope.row.availableTimeSetting.fixedPeriodType === 'VALID_FOR_THE_WEEK'">领取当周有效</div>
            <div v-else>领取当月有效</div>
          </div>
        </template>
      </el-table-column> -->
      ...
      <el-table-column label="操作">
        <template v-slot:default="{ row }">
          <el-link type="primary" @click="addAudit(row)" v-show="row.status.value == 10 || row.status.value == 40" v-np="'COUPON_RULE_CONTENT_ADD'">提交审核</el-link>
          <el-link type="primary" @click="edit(row)" v-show="row.sceneType.value != 3 && (row.status.value == 10 || row.status.value == 30 || row.status.value == 40 || row.status.value == 80)">编辑</el-link>
          <el-link type="primary" @click="del(row)" v-show="row.status.value == 10 || row.status.value == 20 || row.status.value == 40" v-np="'COUPON_RULE_CONTENT_DELETE'">删除</el-link>
          <el-link type="primary" @click="invalid(row)" v-show="row.status.value == 25 || row.status.value == 30 || row.status.value == 50 || row.status.value == 60" v-np="'COUPON_INSTANCE_DISCARD'">作废</el-link>
          <!-- <el-link type="primary" @click="openAndClose(row, 'open')" v-show="row.status.value == 30 || row.status.value == 50">启用</el-link> -->
          <!-- <el-link type="primary" @click="openAndClose(row, 'close')" v-show="row.status.value == 30 || row.status.value == 50">停用</el-link> -->
          <el-link type="primary" @click="copy(row)">复制</el-link>
          <el-link type="primary" @click="showDetail(row)" v-np="['COUPON_RULE_CONTENT_VIEW', 'COUPON_RECEIVED_LIST_VIEW']">查看</el-link>
          <el-link type="primary" @click="$router.push({ path: '/market/couponManager/coupon/receive-detail', query: { couponCode: row.couponCode }})" v-np="['COUPON_INSTANCE_RECEIVED_VIEW', 'COUPON_RECEIVED_DETAIL_VIEW']">领取详情</el-link>
          <el-link type="primary" @click="log(row)" v-np="'COUPON_OPERATION_HISTORY_VIEW'">操作日志</el-link>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  import { reqCouponList, reqAddAudit, reqInvalid, reqDeleteCoupon } from '@/api/coupon'
  ...
  export default {
    ...
    methods: {
      ...
      edit(item) {
        if (item.status.value === 30 || item.status.value === 80) { // 发布中已领完编辑
          let query = {
      ...
      log(row) {
        let query = {
          ...
          rule: row.rule,
          couponType: row.couponType.value
        }
      ...
      del(item) {
        this.$confirm('优惠券删除后无法恢复，请确认是否删除', '优惠券删除', {
          confirmButtonText: '删除',
          cancelButtonText: '取消'
          // type: 'info'
        }).then(() => {
          // 执行提交审核请求
          this._reqDeleteCoupon({ couponCode: item.couponCode })
        }).catch(() => {
        })
      },
      openAndClose(item, type) {
        // 启用停用
        this.$confirm('优惠券作废不影响已领取优惠券，是否确认作废', '优惠券作废', {
          confirmButtonText: '作废',
          cancelButtonText: '取消'
          ...
          // 执行提交审核请求
          this._reqInvalid(item.couponCode, type)
        }).catch(() => {
        })
      },
      // ***请求处理***
      ...
      _reqDeleteCoupon(data) {
        return reqDeleteCoupon(data).then((res) => {
          if (res.code === 200) {
            this.$message({
              message: res.message,
              type: 'success'
