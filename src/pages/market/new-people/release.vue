<template>
  <div class="app-container">
    ...
    <el-table :data="tabledata" border>
          ...
          <el-link type="primary" v-np="'SCENARIO_COUPON_DEVOTE_RECORD_VIEW'" @click="$router.push({ path: '/market/couponManager/coupon/receive-detail', query: { couponCode: row.couponCode }})">发放详情</el-link>
          <el-link type="primary" v-np="'SCENARIO_COUPON_DEVOTE_WAY_DISCARD'" @click="del(row)" v-show="row.sceneStatus.value != 3">移除</el-link>
        </template>
			</el-table-column>
		</el-table>
  </div>
...
<script>
  ...
  import { reqNewPeopleCouponQueryNewPeople, reqInvalid } from '@/api/coupon'
  export default {
    ...
    methods: {
      submitQuery() {
        ...
        this._reqNewPeopleCouponQueryNewPeople(req)
      },
      showDetail(item) {
        let query = {
      ...
      handleReset() {
        this.form = {
          channelType: { value: 'ChannelType_1', name: 'channelType', matchType: MatchType.EQUAL, type: DataType.ENUM },
          code: { value: '', name: 'couponCode', matchType: MatchType.EQUAL, type: DataType.STRING },
          goodsType: { value: '', name: 'scene.sceneStatus', matchType: MatchType.EQUAL, type: DataType.INTEGER }
        }
      },
      _reqNewPeopleCouponQueryNewPeople(data) {
        return reqNewPeopleCouponQueryNewPeople({
