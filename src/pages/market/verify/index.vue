<template>
  <div class="app-container">
    <el-card class="search-content">
      <el-form ref="form" :model="form" label-width="100px" size="mini" class="">
        <el-form-item label="优惠券码">
          <el-input v-model="form.sourceCode.value" placeholder="请输入优惠券码"></el-input>
        </el-form-item>
      ...
      <div class="search-btn">
        <el-button type="primary" size="mini" @click="hanldeBatch" v-np="'COUPON_BATCH_AUDIT'">批量处理</el-button>
    ...
    <el-table :data="tableData" size="mini" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="sourceCode" label="优惠券码"></el-table-column>
      ...
      <el-table-column prop="sceneType" label="投放目标">
        <template slot-scope="scope">
          {{ scope.row.sceneType.describe }}
        </template>
      </el-table-column>
      <el-table-column prop="launchNum" label="发放券码数"></el-table-column>
      <el-table-column prop="totalAmount" label="总发放金额">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作">
        <template v-slot:default="{ row }">
          <el-link v-show="row.auditStatus.value == 1" :disabled="disabledBtn" type="primary" @click="pass(row)" v-np="'COUPON_BATCH_PASS'">通过</el-link>
          <el-link v-show="row.auditStatus.value == 1" :disabled="disabledBtn" type="primary" @click="reject(row)" v-np="'COUPON_BATCH_REJECT'">驳回</el-link>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  import { reqQueryAuditList, reqBatchAudit, reqUserList } from '@/api/coupon'
  export default {
    name: 'man-made',
    components: {
      Pagination
    ...
    methods: {
      ...
      gotoDetail(row) {
        // 跳转优惠券详情
        let query = {
          couponType: row.couponType.value === 1 ? 'DOORFILL' : 'DISCOUNT',
          couponCode: row.sourceCode,
          version: row.version,
          type: 'show'
        }
