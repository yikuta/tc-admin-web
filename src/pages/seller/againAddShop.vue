...
import { asyncAgainAddShop, reqAuditDetail, asyncRegistered } from '@/api/seller'
...
export default {
  ...
  mounted() {
    ...
    this.handleTypeList()
    this.handleTableDetail(this.$route.query.shopCode)
  },
  methods: {
    // 查询门店类型
    handleTypeList() {
    ...
    handleSubmit(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          const {
            shopCode, name, shopType, provinceId, provinceName, cityId, cityName, districtId, districtName, address, legalPerson, companyAddress,
          ...
          asyncAgainAddShop({
            shopCode, name, shopType, provinceId, provinceName, cityId, cityName, districtId, districtName, address,
    ...
    // 查看详情
    handleTableDetail(code) {
      reqAuditDetail(code).then(res => {
        if (res.code === 200) {
          Object.keys(this.form).forEach(key => {
            if (Array.isArray(this.form[key]) && key !== 'shopTimes') {
              let result = key === 'shopSignList' ? res.result : (res.result.shopLicenseVo || '')
              if (result && result[key.substr(0, key.length - 4)]) {
                this.form[key] = [{
                  fileUrl: result[key.substr(0, key.length - 4)],
                  name: result[key.substr(0, key.length - 4)]
                }]
              } else {
                this.form[key] = []
              }
            } else if (key === 'shopTimes') {
              this.form[key] = res.result.times && res.result.times.length ? [res.result.times[0]['beginTime'] || '', res.result.times[0]['endTime'] || ''] : ['', '']
            } else if (key === 'beginTime' || key === 'endTime') {
              this.form[key] = res.result.times && res.result.times.length ? res.result.times[0][key] : ''
              ...
              if (res.result.shopLicenseVo) {
                this.$set(this.form, key, res.result.shopLicenseVo[key] === '永久' || res.result.shopLicenseVo[key] === '长期')
              }
              ...
              this.form[key] = res.result.address
            } else if (key === 'identityCardValidity' || key === 'nonIdentityCardValidity' || key === 'businessTerm') {
              let val = ''
              if (res.result[key] !== '长期' && res.result[key] !== '永久' && res.result.shopLicenseVo && res.result.shopLicenseVo[key]) {
                val = res.result.shopLicenseVo[key].replace(/'年'/g, '')
              ...
              this.form[key] = res.result[key] || (res.result.shopLicenseVo ? res.result.shopLicenseVo[key] : '')
            }
          })
