...
<script>
  import { rejectLeaderAuditCashOut } from '@/api/financial/leader'
  const rules = {
    remark: [
    ...
    methods: {
      ...
      handleConfirm() {
        this.$refs.dialogForm.validate(validate => {
          if (validate) {
            this.$confirm(
              `【${this.row.name}】团长的提现驳回后，会将用户的提现金额【¥ ${this.row.auditAmount}】元解冻，并可以重新提现，确定要驳回吗？`,
              '提现驳回',
              {
                confirmButtonText: '确认驳回',
                cancelButtonText: '我再想想'
              }).then(() => {
              this.rejectLeaderAuditCashOut()
            })
          }
        })
      }
