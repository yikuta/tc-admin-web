    ...
    <div class="describe-table">
      <el-row>
        <el-col :span='6' v-for="(item, index) in describeTable" :key="index">
          {{item.title}} {{item.formatter ? item.formatter(detail) : (detail[item.prop] || '0')}}
        </el-col>
      </el-row>
    </div>
...
<script>
  ...
  import { fetchAuditSubsidyLogs } from '@/api/financial/leader'
    ...
    methods: {
      fetchTableList() {
        fetchAuditSubsidyLogs({
          ...
          if (res.code === 200) {
            const { auditAmount, phoneNumber, name, regimentalCode, pageObj } = res.result
            this.detail = {
              auditAmount, phoneNumber, name, regimentalCode
            }
