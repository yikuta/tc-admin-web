...
<script>
  ...
  import { fetchFinancialFrozenLeaderDetail } from '@/api/financial/leader'
  ...
  export default {
    ...
    methods: {
      fetchTableList() {
        ...
        fetchFinancialFrozenLeaderDetail(postData).then(res => {
          if (res.code === 200) {
            const { phoneNumber, regimentalCode, name, content, pageObj: { itemVOs, totalCount }} = res.result
            this.collection = { phoneNumber, regimentalCode, name, content }
            this.list = itemVOs
