    ...
    >
      ...
      <template slot="more-operates">
        <el-button
          ...
          size="mini"
          v-np="'SHOP_FI_DTL_EXPORT'"
          @click="handleOperatesChange('export')"
        >导出查询结果</el-button>
        <el-button
          type="primary"
          size="mini"
          v-np="'SHOP_SUBSIDY_CREATE'"
...
<script>
  ...
  import { fetchFinancialShopDetail, fetchFinancialShopDetailCollection, exportFinancialShopDetail } from '@/api/financial/shop'

  const initForm = {
    transactionType: { value: '' }, // 测试
    shopBusinessType: { value: '' }, // 业务类型
  ...
  export default {
    ...
    methods: {
      getParams() {
        const {
          transactionType, shopBusinessType, orderType,
        ...
        return filterEmptyParams({
          ...
          shopBusinessType: shopBusinessType.value,
          transactionType: transactionType.value
        })
      },
      fetchFinancialCollection(params) {
        fetchFinancialShopDetailCollection(params).then(res => {
          if (res.code === 200) {
      ...
      fetchTableList(params) {
        fetchFinancialShopDetail(params).then(res => {
          if (res.code === 200) {
      ...
      handleOperatesChange(type) {
        if (type === 'add') {
          ...
          exportFinancialShopDetail(this.getParams()).then(res => {
            if (res.code === 200) {
