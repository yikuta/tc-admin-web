  ...
  <div class="app-container">
    ...
    <div class="express-detail-container">
      ...
      <goods-info
        ...
        @update='handleOrderDetail'
        :orderType='orderType'
        :offlineFlag='orderSign && orderSign.value === 4'
      />
      <brokerage-info :brokerageData='brokerageData'/>
      <pay-info
        :payStatus='payStatus'
...
import { fetchOrderDetail, fetchCommissionByOrder, queryLoadOrderLogs } from '@/api/order'
export default {
  ...
  methods: {
      ...
      fetchOrderDetail(orderNum).then(res => {
        if (res.code === 200 && res.result) {
          ...
          this.deliveryInfo.map(item => {
            ...
            } else if (item.key === 'selfMentionTime') {
              item.value = res.result.selfMentionTime ? res.result.selfMentionTime.split(' ')[0] : '-'
            } else {
              item.value = res.result[item.key]
            }
          })
    ...
    handleCommission() {
      const { orderNum } = this.$route.query
      fetchCommissionByOrder(orderNum).then(res => {
        if (res.code === 200) {
          this.brokerageData = res.result || []
        } else {
          this.brokerageData = []
        }
      }).catch(() => {
