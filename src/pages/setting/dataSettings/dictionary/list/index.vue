    ...
    <el-table :data="list">
     ...
     <!-- <el-table-column prop="operator" label="操作人"></el-table-column>-->
      <el-table-column label="操作" width="224px">
        <template slot-scope="scope">
          <el-button type="text">
            <router-link :to="{ name: 'dictionaryFields', query: { pid: scope.row.id }}">字典值</router-link>
          </el-button>
          <el-button type="text" @click="handleEditDictionary(scope.row)" v-np="'VARIABLE_CONF_EDIT'">编辑</el-button>
          <el-button type="text" @click="updateFieldStatus(scope.row)" v-np="scope.row.enable ?'VARIABLE_CONF_DISABLE':'VARIABLE_CONF_ENABLE'">{{ scope.row.enable ? '停用' : '启用' }}</el-button>
          <el-button type="text" v-np="'COMMON_OPERATE_LOG'">
            <router-link :to="{ path: 'dictionaryLogs', query: { code: scope.row.code, name: scope.row.name, type: 'list' }}">操作日志</router-link>
          </el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      updateFieldStatus(row) {
        const message = row.enable ? '数据字典停用后将不能在各地方使用，确定要停用吗' : '数据字典启用后有调用的位置才能正常使用，确定要启用吗？'
        this.$confirm(message, '提示', {
          ...
          updateDictionary({ id: row.id, enable: !row.enable }).then((res) => {
            if (res.code === 200) {
              this.$message.success(row.enable ? '停用数据字典成功～' : '启用数据字典成功~')
              this.fetchList()
            } else {
