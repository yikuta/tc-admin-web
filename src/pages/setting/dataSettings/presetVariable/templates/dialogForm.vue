<template>
  ...
  >
    <el-form :model="form" :rules="rules" size="mini" ref="dictionaryForm">
      <el-form-item label="变量编码" label-width="80px" prop="code">
        <el-input v-model="form.code" autocomplete="off" placeholder="请输入变量编码" />
      </el-form-item>

      <el-form-item label="变量名称" label-width="80px" prop="name">
        <el-input v-model="form.name" autocomplete="off" placeholder="请输入变量名称" />
      </el-form-item>

      <el-form-item label="变量值" label-width="80px" prop="value">
        <el-input v-model="form.value" autocomplete="off" placeholder="请输入变量值" />
      </el-form-item>
    ...
    methods: {
      handleSubmit() {
        this.$refs.dictionaryForm.validate(validate => {
          if (validate) {
            this.form.isOneToMany = 0 // 变量
            this.$confirm(`确定要${this.form.id ? '编辑' : '添加'}变量："${this.tempRow.name || this.form.name}" 吗？`, '提示', {
              ...
              promise.then((res) => {
                if (res.code === 200) {
                  this.$message.success(this.form.id ? '数据变量修改成功~' : '数据变量添加成功~')
