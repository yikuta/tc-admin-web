    ...
    <el-table 
      ...
      <el-table-column label="操作">
        <template v-slot:default="{ row }">
          <el-link type="primary" @click="handleDeal(row)" v-if="row.afterSaleState&&row.afterSaleState.value === 1" v-np='"AFTER_SALE_REQ_DETAIL_AUDIT"'>去处理</el-link>
          <el-link type="primary" @click="handleDeal(row)" v-else-if="row.afterSaleState&&row.afterSaleState.value === 5" v-np='"AFTER_SALE_REQ_LIST_AGAIN_VIEW"'>重新处理</el-link>
          <el-link type="primary" @click="handleDetail(row)" v-np='"AFTER_SALE_REQ_DETAIL_VIEW"' v-else>查看详情</el-link>
        </template>
      </el-table-column>
    </el-table>
  </div>
