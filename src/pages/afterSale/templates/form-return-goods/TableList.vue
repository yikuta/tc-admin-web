<template>
  <div class="tableList">
    <el-table 
      ...
      <el-table-column prop="generateNum" label="退货单号"></el-table-column>
      ...
      <el-table-column prop="returnOfGoodsState" label="退货状态">
        <template v-slot:default="{row}">{{row['returnOfGoodsState'] | statusDescrible}}</template>
      </el-table-column>
      ...
      <el-table-column label="操作">
        <template v-slot:default="{ row }">
          <el-link type="primary" @click="handleDetail(row)" v-np='"AFTER_SALE_RETURN_ORDER_DETAIL_VIEW"'>查看详情</el-link>
        </template>
      </el-table-column>
    </el-table>
  </div>
