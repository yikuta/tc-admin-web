<template>
  <div class="tableList">
    <el-table 
      ...
      <el-table-column prop="generateNum" label="发券单号"></el-table-column>
      ...
      <el-table-column prop="reCouponState" label="发券状态">
        <template v-slot:default="{row}">{{row['reCouponState'] | statusDescrible}}</template>
      </el-table-column>
      ...
      <el-table-column label="操作">
        <template v-slot:default="{ row }">
          <el-link type="primary" @click="handleDetail(row)" v-np='"AFTER_SALE_EXTRA_COUPON_ORDER_DETAIL_VIEW"'>查看详情</el-link>
        </template>
      </el-table-column>
    </el-table>
  </div>
