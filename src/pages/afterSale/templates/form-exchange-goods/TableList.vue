<template>
  <div class="tableList">
    <el-table 
      ...
      <el-table-column prop="generateNum" label="换货单号"></el-table-column>
      ...
      <el-table-column prop="exchangeGoodsState" label="换货状态">
        <template v-slot:default="{row}">{{row['exchangeGoodsState'] | statusDescrible}}</template>
      </el-table-column>
      ...
      <el-table-column label="操作">
        <template v-slot:default="{ row }">
          <el-link type="primary" @click="handleDetail(row)" v-np='"AFTER_SALE_EXCHANGE_ORDER_DETAIL_VIEW"'>查看详情</el-link>
        </template>
      </el-table-column>
    </el-table>
  </div>
