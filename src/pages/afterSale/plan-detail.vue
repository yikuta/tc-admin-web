    ...
    handleDetail() {
      ...
      fn(afterSaleNum, generateNum).then(res => {
        if (res.code === 200) {
          ...
          this.orderState = result.baseAfterSaleDetailInfo.baseStateVo || { value: '', describe: '' }
          ...
          this.afterSaleInfo.map(item => {
            if (
              ...
              item.value = result.baseAfterSaleDetailInfo[item.key] || result.baseAfterSaleDetailInfo[item.key] === 0 ? result.baseAfterSaleDetailInfo[item.key] : '-'
            }
          })
          // 仓库信息
