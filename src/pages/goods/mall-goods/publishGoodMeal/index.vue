<template>
  <div class="app-container">
    ...
    <div class="action-area">
      ...
      <el-button v-if="currentStep === 2" type="primary" :loading="submitLoading" @click="handleSave">保存</el-button>
    </div>
  </div>
</template>
    ...
    data() {
      return {
        ...
        form: {
          ...
          freightId: '', // 物流模板
          packageType: 2, // 1-自由组合套餐 ｜ 2-固定组合套餐
          discount: false, // 是否折扣
          baseSale: 0, // 基础销量
          sort: 0, // 展示排序
          shelfOnMode: 1, // 上架模式
    ...
    methods: {
      handlePrev() {
        if (this.currentStep > 0) {
          this.$confirm('返回上一步将可能清空本页已填写的所有信息，确定要返回吗？', '提示', {
        ...
        fetchGoodMealById(this.goodId).then(res => {
          ...
          channelCommodityPackageList.forEach(item => {
            const {
              ...
              minCollocationPrice, maxCollocationPrice, minSalePrice, maxSalePrice
            } = item
            const newItem = {
              ...
              salePrice: priceSection(minSalePrice, maxSalePrice),
      ...
      handleSave() {
        ...
        const channelCommodityPackageList = [{
          ...
          channelCommoditySpecificationList: this.mainGood.baseCommoditySpecificationList.map(item => {
            item.baseSkuId = item.baseSkuId || item.id
