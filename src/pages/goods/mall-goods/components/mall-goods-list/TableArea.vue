<template>
  <div class="wrapper">
    <el-table :data="tableData" style="width: 100%" :cell-style="{ textAlign:'center' }" :header-cell-style="{ textAlign:'center' }" @selection-change="handleSelectionChange" size="mini">
      ...
      <el-table-column prop="stock" label="库存">
        <template slot-scope="{ row }">
          <div>
            ...
            <div>限卖量:{{row['totalSaleLimit']}}</div>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="freightName" label="物流模板">
        <template v-slot:default="{row}">{{row.transfer?'包邮': row.freightStr}}</template>
      </el-table-column>
      <el-table-column prop="special" label="特殊商品" :formatter="formateSpecial"></el-table-column>
      <el-table-column prop="commodityType" label="商品类型">
        <template slot-scope="{row}">
          <span>{{row.commodityType.value===1?'单品':'套餐'}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="160">
        <template v-slot:default="{ row }">
          <el-link v-np="'CHANNEL_GOODS_EDIT'" type="primary" @click="handleEdit(row)">编辑</el-link>
          <el-link v-np="'CHANNEL_GOODS_SHELF_ON'" type="primary" @click="handlePutOn(row)" v-if="isAbledPutOn(row)">上架</el-link>
          <el-link v-np="'CHANNEL_GOODS_SHELF_OFF'" type="primary" @click="handlePutOff(row)" v-else>下架</el-link>
          <el-link v-np="'CHANNEL_GOODS_DEL'" type="danger" @click="handleRemove(row)">删除</el-link>
          <el-link v-np="'CHANNEL_GOODS_PRICE_EDIT'" type="primary" v-if="row.commodityType.value === 1" @click="handlePriceSetting(row)">价格设置</el-link>
          <el-link v-np="'CHANNEL_GOODS_STORE_EDIT'" type="primary" v-if="row.commodityType.value === 1" @click="handleStoreSetting(row)">库存设置</el-link>
          <el-link v-np="'CHANNEL_GOODS_GET'" type="primary" @click="handlePreview(row)">商品预览</el-link>
          <el-link v-np="'COMMON_OPERATE_LOG'" type="primary" @click="handleCheckLog(row)">操作日志</el-link>
        </template>
      </el-table-column>
      <template #empty>没有查询到商品数据！</template>
    </el-table>
    <div class="action-area">
      <el-button v-np="'CHANNEL_GOODS_SHELF_BATCH_ON'" type="primary" size="mini" @click="handleBatchPutOn">批量上架</el-button>
      <el-button v-np="'CHANNEL_GOODS_SHELF_BATCH_OFF'" type="primary" size="mini" @click="handleBatchPutOff">批量下架</el-button>
    </div>
