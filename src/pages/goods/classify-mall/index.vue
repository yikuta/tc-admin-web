        ...
        <el-card class="card-">
          <div class="top-search-bar">
            ...
            <el-button
              ...
              size="mini"
              @click="handleAdd(j)"
              v-np="j === 0 ?'CHANNEL_CATE_ONE_ADD':'CHANNEL_CATE_TWO_ADD'"
          ...
          <div class="list-box" v-if="transfer.length>0 && transfer[j] && transfer[j].length>0">
            <div v-for="(item, index) in transfer[j]" :key="index" class="text item" :class="[{'active': classList[j] && item.id === classList[j]}]"
              ...
              <div class="btn-box">
                ...
                <el-button v-np="'CHANNEL_CATE_UPDATE'" type="text" @click.stop="handleEdit(item, j)">编辑</el-button>
                <el-button v-np="item.enable ? 'CHANNEL_CATE_DISABLE' : 'CHANNEL_CATE_ENABLE'" type="text" @click.stop="handleStatus(item, j)">{{item.enable?'停用':'启用'}}</el-button>
                <el-button v-np="'CHANNEL_CATE_DEL'" type="text" @click.stop="handleDelete(item, j)" style="color:#D9001B">删除</el-button>
              </div>
            </div>
          </div>
