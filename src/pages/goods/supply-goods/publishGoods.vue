  ...
  methods: {
    ...
    handleSave() {
      if (!this.isChannel) {
        ...
        req.channelCommodityMediaList.push({ type: 4, cover: false, typeValue: headerContent.value, htmlValue: headerContent.html })
        req.channelCommodityMediaList.push({ type: 5, cover: false, typeValue: bottomContent.value, htmlValue: bottomContent.html })
        if (this.$refs['editGoods'].form.imageUrl) {
          req.thumbnailImage = this.$refs['editGoods'].form.imageUrl
          req.resetCoverImage = 1
          req.channelCommodityMediaList.push({ type: 1, cover: true, typeValue: req.thumbnailImage })
        }
        ...
        reqUpdateMallGoods(req)
          .then(res => {
            if (res.code === 200) {
              ...
              this.$router.push({ path: '/goods/supplyGoods' })
            } else {
