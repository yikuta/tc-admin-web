<template>
  <div class="wrapper">
    ...
    <div class="current-goods">
      您当前选择的商品是：
      <span>{{currentSelected.commodityName}}</span>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      this._reqBaseGoodsList().then(() => {
        if (this.currentGoodsId || this.currentGoodsId === 0) {
          this.currentSelected = this.tableData.filter(item => {
            if (item.id === this.currentGoodsId) {
              return true
            }
            return false
          })[0]
  ...
  methods: {
    ...
    handleCurrentChange(currentRow) {
      ...
      if (currentRow) {
        ...
        this.$emit('selectedCurrentGoods', currentRow)
      } else {
        this.$emit('clearSelectedCurrentGoods')
