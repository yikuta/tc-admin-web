<template>
  <el-dialog title="关联规格" :visible.sync="dialogVisible" @close='handleCancel' :close-on-click-modal='false'>
    <el-form class="" size="small" label-width="100px">
      <el-form-item label="搜索规格">
        <el-select
          v-model="selectedId"
          filterable
          clearable
...
<script>
  import { fetchBaseSpecificationList, addBaseCategorySpecification } from '@/api/baseCategory'
  ...
  export default {
    ...
    data() {
      return {
        ...
        pageSize: 10,
        form: {
          name: { value: '', name: 'name', matchType: MatchType.CONTAIN }, // 规格名称
          enable: { value: true, name: 'enable', matchType: MatchType.EQUAL, type: DataType.BOOLEAN } // 状态
        },
        groups: [],
        selectedId: '',
        btnLoading: false
    ...
    methods: {
      ...
      handleChange(value) {
        if (value) {
          ...
          if (this.groups.length + this.len >= 10) {
            this.$message.error('一个分类最多只能关联10个规格哦')
          } else {
      ...
      handleSave() {
        ...
        let sorts = this.groups.map((item, index) => this.maxSpecsSort + index + 1)
        if (ids && ids.length > 0) {
          this.btnLoading = true
          addBaseCategorySpecification({
            baseCategoryId: this.$route.query.id,
            baseSpecificationIds: ids,
            sorts
          }).then(res => {
            this.btnLoading = false
            if (res.code === 200) {
              this.$message.success('关联规格成功~')
              this.handleCancel()
              this.$emit('update')
            }
          }).catch(() => {
