...
<script>
import { assembleActivityList } from './components'
import Pagination from '@/components/Pagination'
import searchTool from '@/utils/searchTool'
import { MatchType, DataType } from '@/constants/search'
import { reqAssembleActivityList } from '@/api/assembleGoods'

export default {
  name: 'assembleActivityList',
  components: {
    Pagination,
    ...assembleActivityList
  },
  data() {
    return {
      currentPage: 1,
  ...
  methods: {
    ...
    onSearch() {
      ...
      if (req.cityCategory && req.cityCategory.value && req.cityCategory.value.length) {
        req.province = { value: ((req.cityCategory || {}).value || [])[0], name: 'businessCity.provinceCode', matchType: MatchType.EQUAL, type: DataType.STRING }
        req.city = { value: ((req.cityCategory || {}).value || [])[1], name: 'businessCity.cityCode', matchType: MatchType.EQUAL, type: DataType.STRING }
      }
      delete req.cityCategory
      this._reqAssembleActivityList(searchTool(req))
    },
    onQuery() {
      this.currentPage = 1
    ...
    // ***请求处理***
    _reqAssembleActivityList(data) {
      return reqAssembleActivityList({
