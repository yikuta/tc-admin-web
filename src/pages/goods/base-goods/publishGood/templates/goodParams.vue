      ...
      <div class="content">
        <div v-for="param in item.baseParamOptionList" :key="param.id">
          ...
          <template v-else-if="param.paramType.value === 4">
            <el-form-item  :label-width="param.name | setLabelWidth" :label="param.name" :error="param.error">
              <el-input type="text" v-model="param.selectedValue" @blur="handleInputBlur(param)" placeholder="请输入文本"></el-input>
            </el-form-item>
          </template>
          ...
          <template v-else-if="param.paramType.value === 5">
            <el-form-item :label-width="param.name | setLabelWidth" :label="param.name" :error="param.error">
              <el-input type="textarea" v-model="param.selectedValue" @blur="handleInputBlur(param)" placeholder="请输入文本"></el-input>
            </el-form-item>
          </template>
        </div>
      </div>
    ...
    methods: {
      handleInputBlur(param) {
        if (param.paramType.value === 4) {
          this.$set(param, 'error', param.selectedValue && param.selectedValue.length > 100 ? '单行文本不超过100个字符' : '')
        } else if (param.paramType.value === 5) {
          this.$set(param, 'error', param.selectedValue && param.selectedValue.length > 500 ? '多行文本不超过500个字符' : '')
        }
      }
    }
  }
