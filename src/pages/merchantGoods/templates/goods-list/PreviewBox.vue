        ...
        <div class="mobile-content" >
          <el-carousel indicator-position="none">
            <el-carousel-item v-for="(item, index) in swipper" :key="index">
              <video ref="video" :src="item.url" controls :style="{width:'100%',height:'300px'}" v-if="item.type==='video'"/>
              <img :src="item.url" style="width:100%;height:300px" v-else/>
            </el-carousel-item>
          </el-carousel>
          <div class="good-info">
...
<script>
  export default {
    ...
    computed: {
      swipper() {
        const { headImgs, videoCover, videoUrl } = this.previewDetail
        let arr = []
        if (headImgs && headImgs.length) {
          headImgs.map(item => {
            arr.push({ url: item, type: 'img' })
          })
        }
        if (videoCover) {
          arr.push({ url: videoCover, type: 'img' })
        }
        if (videoUrl) {
