<template>
  <div class="app-container">
    ...
    <el-row style="margin-bottom:50px">
      <el-col :span="7" :offset="1">消息标题：{{$route.query.title}}</el-col>
      <el-col :span="16" v-if="$route.query.type !== 'PM-SOUND'">消息内容：{{$route.query.content}}</el-col>
    </el-row>
...
export default {
  ...
  computed: {
    info() {
      if (this.$route.query.type === 'SM') {
        ...
        return '站内信信息'
      } else if (this.$route.query.type === 'PM-TEXT') {
        return 'push消息（文字）信息'
      } else {
        return 'push消息（语音）信息'
      }
  ...
  methods: {
    handleDataList() {
      let obj = {
        taskId: { value: this.$route.query.id, name: 'taskId', matchType: MatchType.EQUAL, type: DataType.NUMBER }
      }
      let templateType = ''
      if (this.$route.query.type === 'PM-TEXT' || this.$route.query.type === 'PM-SOUND') {
        templateType = 'PM'
      } else {
